...
def get_image(entry):
    ...
    if not url or "unsplash" in url:
        try:
            ...
            pass # Silent failure on timeout/connection error

    # 3. SPEED & SPINNER FIX
    if url and isinstance(url, str) and len(url) > 10:
        # Only strip query params if it's not ESPN (ESPN relies on query params for image delivery)
        if "espn" not in url:
            url = url.split('?')[0] 
...
for src in SOURCES:
    ...
    try:
        # Override feedparser's default bot user-agent
